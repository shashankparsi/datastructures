...
void *poly_add(struct node *head , struct node *head1)
{
...
{
res=new_insert(res,first->coffecient + second->coffecient,first->exponent);
first=first->link;
second=second->link;
}
else if(first->exponent > second->exponent)
{
res=new_insert(res,first->coffecient,first->exponent);
first=first->link;
}
else if(first->exponent < second->exponent)
{
res=new_insert(res,second->coffecient,second->exponent);
second=second->link;
}
}
while(first!=NULL)
...
/*struct node *addterm(struct node *res,float coff)
{
...
printf("********* please maintain polynomial exponents in descending order *******\n");
